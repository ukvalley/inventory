...
@section('breadcrumbs')
    @php
        $parentChildBreadcrumbs = App\Models\Admin\Users::buildParentChildrenBreadcrumbs(Request()->id);
    @endphp
...
@endsection
@section('backBtn')
<a href="@if(Request()->id) {{route("admin.users.index",App\Models\Admin\Users::find(Request()->id)->admiko_parent_child)}} @else {{route("admin.home")}} @endif"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card users_index admikoIndex">
            ...
            <div class="tableLayout pb-2">
                    ...
                    @foreach($tableData as $data)
                        <tr>
							...
							<td class="text-nowrap text-center no-sort"><a href="{{route("admin.users.index",[$data->id])}}"><i class="fas fa-stream fa-fw"></i></a></td>
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['users_allow']))
                        <a href="{{route('admin.users.create',Request()->id??0)}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.users.delete",Request()->id??"")}}">
            @method('DELETE')
            @csrf
